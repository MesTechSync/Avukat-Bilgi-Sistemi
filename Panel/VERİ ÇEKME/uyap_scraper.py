...
# -*- coding: utf-8 -*-
"""
UYAP Emsal Karar Arama Sistemi Veri Çekme Aracı
Bu araç, emsal.uyap.gov.tr sitesinden yasal sınırlar içinde veri çekmek için tasarlanmıştır.
"""
...
class UYAPScraper:
    def __init__(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('uyap_scraper.log', encoding='utf-8'),
    ...
    def search_decisions(self, keyword="", court_unit="", date_from="", date_to="", 
        ...
        """
        self.logger.info(f"Karar arama başlatılıyor: {keyword}")
        ...
        try:
            ...
            if search_form:
                ...
                # Mahkeme seçimi (eğer belirtilmişse)
                if court_unit:
                    # Mahkeme seçimini yap
                    pass
    ...
    def _extract_decision_data(self, row):
        """Tek bir karar verisini çıkarır"""
        data = {}
    ...
    def get_decision_detail(self, decision_url):
        """Karar detay sayfasından tam metni çeker"""
        try:
            self._rate_limit()
            ...
            # Karar metnini bul
            content_elem = soup.find('div', class_='decision-content') or soup.find('div', class_='content')
            if content_elem:
    ...
    def save_to_csv(self, results, filename=None):
        ...
        if not filename:
            filename = f"uyap_kararlar_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def save_to_json(self, results, filename=None):
        ...
        if not filename:
            filename = f"uyap_kararlar_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    if results:
        print(f"{len(results)} karar bulundu")
        ...
        for i, result in enumerate(results[:3]):
            ...
            print(f"   Mahkeme: {result.get('court', 'N/A')}")
    else:

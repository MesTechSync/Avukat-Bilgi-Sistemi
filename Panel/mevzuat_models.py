...
class MevzuatDateRangeRequest(BaseModel):
    """Request model for collecting legislation within specific year range."""
    start_year: int = Field(..., ge=1900, le=2030, description="Start year for legislation collection (inclusive)")
    end_year: int = Field(..., ge=1900, le=2030, description="End year for legislation collection (inclusive)")
    mevzuat_tur_list: List[MevzuatTurEnum] = Field(
        default_factory=lambda: ["KANUN", "CB_KARARNAME", "YONETMELIK", "CB_YONETMELIK", "CB_KARAR", "CB_GENELGE", "KHK", "TUZUK", "KKY", "UY", "TEBLIGLER"],
        description="Filter by legislation types to collect."
    )
    max_documents_per_year: int = Field(100, ge=1, le=1000, description="Maximum documents to collect per year")
    include_full_text: bool = Field(False, description="Whether to include full text content of documents")
    export_format: Literal["json", "csv", "xlsx"] = Field("json", description="Export format for collected data")
...
class MevzuatCollectionResult(BaseModel):
    """Result model for year-range legislation collection."""
    request_parameters: MevzuatDateRangeRequest
    total_documents_collected: int
    documents_by_year: Dict[str, int] = Field(description="Number of documents collected for each year")
    collection_summary: Dict[str, Any] = Field(description="Summary statistics of the collection")
    documents: List["MevzuatDocument"] = Field(description="Collected legislation documents")
    export_file_path: Optional[str] = Field(None, description="Path to exported file if requested")
